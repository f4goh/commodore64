...
//    baseFrame,speed,damage,color,bbox_x,bbox_y,bbox_w,bbox_h
    { 3        ,6    ,100   ,7    ,0     ,8     ,16    ,5 },  
    { 21       ,3    ,100   ,7    ,10    ,8     ,6     ,4 }     // #1: enemy bullet  
};
...
//    color       ,baseFrame,numFrames,animSpeed,hp  ,points,bbox_x,bbox_y,bbox_w,bbox_h,bulletTypeID,bulletPeriod,bulletAngle
    { VCOL_LT_RED ,4        , 6       ,10       ,100 ,100   ,0     ,0     ,24    ,14    ,-1          ,0           ,0   },
    { VCOL_LT_BLUE,10       , 3       ,10       ,100 ,200   ,0     ,0     ,24    ,21    ,1           ,20          ,19 }
};
...
EnemyBullet enemyBulletPool[MAX_ENEMY_BULLETS];
...
void spawn_enemy_bullet(int16_t x, uint8_t y, uint8_t bulletTypeID,uint8_t typeID);
    ...
    for (uint8_t i = 0; i < MAX_BULLETS; i++) {
        if (bulletPool[i].active) {
            ...
            if (bulletPool[i].x > 344) {
                ...
                vspr_move(i + 3, bulletPool[i].x, bulletPool[i].y);
            }
        }
    }
}
...
void update_enemy_bullets() {
    for (uint8_t i = 0; i < MAX_ENEMY_BULLETS; i++) {
        EnemyBullet *eb = &enemyBulletPool[i];
...
void check_bullets_player_collisions() {
    ...
    for (uint8_t b = 0; b < MAX_ENEMY_BULLETS; b++) {
        EnemyBullet *eb = &enemyBulletPool[b];
