...
#define TITLE_FLASH_DURATION 16
...
char * const Charset3 = (char *)  0xC800;
...
uint16_t scrollX=8*CAMERA_X0-1;
...
//  SpwnX,Typ,Ct,Sp,Sx ,Sy ,vx,PathID,     
    { 344,0  ,1 ,0 ,344,110,2 ,PATH_SINUSOID}, // 43
    { 440,2  ,1 ,0 ,344, 90,2 ,PATH_LINEAR},   // 55
    { 680,1  ,3 ,20,344,130,2 ,PATH_LINEAR},   // 85
    { 744,1  ,3 ,20,344,90 ,2 ,PATH_LINEAR},   // 93
    { 864,3  ,1 ,0 ,344,90 ,2 ,PATH_LINEAR},   // 108
    { 984,4  ,1 ,0 ,344,90 ,2 ,PATH_LINEAR},   // 123
    {1272,5  ,1 ,0 ,255,140 ,0 ,PATH_NONE},
    {1272,6  ,1 ,0 ,279,140,0 ,PATH_NONE},
    {1272,7  ,1 ,0 ,303,140,0 ,PATH_NONE},
    {1272,8  ,1 ,0 ,255,161,0 ,PATH_NONE},
    {1272,9  ,1 ,0 ,279,161,0 ,PATH_NONE},
    {1272,10 ,1 ,0 ,303,161,0 ,PATH_NONE},
...
//  col,fr,nF,as,hp,pts,bx,by,bw,bh,bID,bP,bA,pupType      ,isBoss
    {10,4 ,6 ,10,10,100,0 ,0 ,24,14,-1 ,0 ,0 ,PWR_NONE     ,0},
    {14,10,3 ,10,10,200,0 ,0 ,24,21,1  ,20,19,PWR_NONE     ,0},
    { 6,27,1 ,0 ,10,100,0 ,0 ,22,20,-1 ,0 ,0 ,PWR_SATELLITE,0},
    { 6,27,1 ,0 ,10,100,0 ,0 ,22,20,-1 ,0 ,0 ,PWR_LASER    ,0},
    { 6,27,1 ,0 ,10,100,0 ,0 ,22,20,-1 ,0 ,0 ,PWR_WAVE_FIRE,0},
    {12,33,1 ,0 ,40,500,14,9 ,10,12,-1 ,0 ,0 ,PWR_NONE     ,1 },
    {12,34,1 ,0 ,40,500,0 ,0 ,24,21,-1 ,0 ,0 ,PWR_NONE     ,1 },
    {12,35,1 ,0 ,40,500,0 ,0 ,24,21,-1 ,0 ,0 ,PWR_NONE     ,1 },
    {12,36,1 ,0 ,40,500,14,0 ,10,15 ,1 ,50,20,PWR_NONE     ,1 },
    {12,37,1 ,0 ,40,500,0 ,0 ,24,21,-1 ,0 ,0 ,PWR_NONE     ,1 },
    { 8,38,1 ,0 ,40,500,0 ,0 ,24,16,-1 ,0 ,0 ,PWR_NONE     ,1 }
};
...
char msg[] = {201, 202, 203, 202, 201, 200, 204, 205,
              206, 207, 201, 202, 208, 202, 209, 0};

const char TitleFont[] = { 
    #embed ctm_chars lzo "../assets/title_logo.ctm" 
};

const char CreditsMap[] = { 
    #embed ctm_map8 "../assets/title_credits.ctm" 
};

const char LogoMap[] = { 
    #embed ctm_map8 "../assets/title_logo.ctm" 
};

int8_t logo_map_y = 14;
uint8_t scrollY = 0;
uint16_t title_timer = TITLE_FLASH_DURATION;
uint8_t  title_color = 3;
uint8_t draw_credits_done = 0;

const char StarsMap[] = { 
    #embed ctm_map8 "../assets/stars.ctm" 
};
...
void init_gamestate_title();
void draw_logo_row();
void scroll_logo_down();
void draw_credits();
...
void draw_stars();
void hard_scroll_stars();
void fill_column_stars();
...
int main() {
    ...
    while (true) {
        ...
        case GAME_TITLE:
            ...
            rirq_data(&irq_charset_change,0,2);
            rirq_data(&irq_charset_change,1,VCOL_BLACK);
            rirq_data(&irq_charset_change,2,VCOL_WHITE);
            rirq_data(&irq_charset_change,3,VCOL_BLUE);
            rirq_data(&irq_charset_change,4,VIC_CTRL2_MCM);
            rirq_data(&irq_scroll, 0,VIC_CTRL2_MCM);
            rirq_data(&irq_panel, 0,2);
            ...
            title_timer--;
            if(title_timer==0) {
                title_timer = TITLE_FLASH_DURATION;
...
void scroll_left() {
    if (scrollX < MAP_MAX_X) {
        ...
        scrollX++;
        starsX += 0.5*256.0;
        uint8_t value = (((starsX >> 8) & 7) ^ 7) | VIC_CTRL2_MCM;
        rirq_data(&irq_charset_change, 4, value);
        rirq_data(&irq_scroll, 0, ((scrollX & 7) ^ 7) | VIC_CTRL2_MCM);
        rirq_data(&irq_panel, 2, VIC_CTRL2_MCM);
...
void init_video() {
    ...
    oscar_expand_lzo(Charset3,TitleFont);
        ...
        if (enemyPool[eIdx].isBoss == 1) {
            for(uint8_t i=0; i<MAX_ENEMIES_POOL; i++) {
                if (enemyPool[i].active==1 && enemyPool[i].isBoss==1 && enemyPool[i].state != STATE_EXPLODING) {
                    ...
                    enemyPool[i].baseFrame = 13;
                    enemyPool[i].currentFrame = 0;
                    enemyPool[i].maxFrames = 8;
                    enemyPool[i].animTimer = 0;
...
void check_player_background_collision() {
    ...
    if ( (Screen[row1 + tx1] > 0 && Screen[row1 + tx1] < 210)  || 
         (Screen[row1 + tx2] > 0 && Screen[row1 + tx2] < 210)  || 
         (Screen[row2 + tx1] > 0 && Screen[row2 + tx1] < 210)  || 
         (Screen[row2 + tx2] > 0 && Screen[row2 + tx2] < 210) ) 
    {
        spawn_player_explosion();
    }
}
...
void init_irq() {
    ...
    rirq_build(&irq_scroll,1);
    rirq_write(&irq_scroll, 0, &vic.ctrl2, 0);
    rirq_set(9, 151, &irq_scroll);
...
uint8_t check_background_collision(uint16_t x, uint8_t y) {
    ...
    if (Screen[offset] > 0 && Screen[offset] < 210) {
        return 1;
    }
...
void check_player_powerup_collision() {
    ...
    } else if (p->typeID == PWR_SATELLITE) {
...
void spawn_satellite(uint16_t x, uint16_t y) {
    ...
    vspr_set(1, x, y, sprite_base + 29, VCOL_RED);
}
...
void satellite_update() {
    ...
    uint8_t state = s->state;

    if (state == SAT_STATE_DOCKED_FRONT) {
        s->x = player_ship.x + 18; 
        s->y = (uint16_t)(player_ship.y) << 8;  
    } 
    else if (state == SAT_STATE_DOCKED_BACK) {
        s->x = player_ship.x - 22; 
        s->y = (uint16_t)(player_ship.y - 2) << 8;
    }
    else if (state == SAT_STATE_RELEASE || state == SAT_STATE_RECALL) {
        s->x += s->speedX;
        ...
        uint8_t idx = s->pathIdx;
        if (++idx >= 64) idx = 0; 
        s->pathIdx = idx;

        int16_t dy = (int16_t)sineTableFixed[idx] << 1; 
    ...
    uint16_t px = s->x;
    uint8_t  py = (uint8_t)(s->y >> 8);
    vspr_move(1, px, py);

    if (++s->animTimer >= s->animSpeed) {
        s->animTimer = 0;
        uint8_t frame = s->currentFrame + 1;
        if (frame >= s->maxFrames) frame = 0;
        s->currentFrame = frame;

        vspr_image(1, sprite_base + s->base_frame + frame);
    }
}
...
void check_player_satellite_docking() {
    ...
    if (!s->active || state == SAT_STATE_DOCKED_FRONT || state == SAT_STATE_DOCKED_BACK) return;
    ...
    uint16_t s_left   = s->x + s->bbox_x;
    uint16_t s_right  = s_left + s->bbox_w;
    uint8_t  s_top    = (uint8_t)(s->y >> 8) + s->bbox_y;
    uint8_t  s_bottom = s_top + s->bbox_h;

    if (s_left > p_right || s_right < p_left || s_top > p_bottom || s_bottom < p_top) 
        return;

    uint16_t p_mid = p_left + (player_ship.bbox_w >> 1);

    if (s_left > p_mid) {
        s->state = SAT_STATE_DOCKED_FRONT;
    } else {
        s->state = SAT_STATE_DOCKED_BACK;
    }
}
...
void check_satellite_launch() {
    ...
    if (!s->active || s->state == SAT_STATE_RELEASE) return;
    ...
    if (key_pressed(KSCAN_SPACE)) {
        ...
        if (state == SAT_STATE_DOCKED_FRONT) {
            s->speedX = 4;
            s->state = SAT_STATE_RELEASE;  
        } 
        else if (state == SAT_STATE_DOCKED_BACK) {
            s->speedX = -4;
            s->state = SAT_STATE_RELEASE;
        }
        else if (state == SAT_STATE_FREE) {
            if (s->x > player_ship.x)
                s->speedX = -4;
            else
                s->speedX = 4;
            s->state = SAT_STATE_RECALL;
        }
    }
}
...
void check_bullets_satellite_collisions() {
    ...
    Bullet *eb = enemyBulletPool;

    for (uint8_t b = 0; b < MAX_ENEMY_BULLETS; b++) {
        if (!eb->active) { eb++; continue; }

        uint16_t b_left = (eb->x >> 6) + eb->bbox_x;
        
        if (b_left > s_right) { eb++; continue; }

        uint16_t b_right = b_left + eb->bbox_w;
        if (b_right < s_left) { eb++; continue; }

        uint8_t b_top = (uint8_t)(eb->y >> 6) + eb->bbox_y;
        if (b_top > s_bottom) { eb++; continue; }

        uint8_t b_bottom = b_top + eb->bbox_h;
        if (b_bottom < s_top) { eb++; continue; }
...
void check_satellite_enemy_collision() {
    ...
    for (uint8_t i = 0; i < MAX_ENEMIES_POOL; i++) {
        if (!e->active || e->state == STATE_EXPLODING) { e++; continue; }

        uint16_t e_left = e->x + e->bbox_x;
        if (e_left > s_right) { e++; continue; }

        uint16_t e_right = e_left + e->bbox_w;
        if (e_right < s_left) { e++; continue; }

        uint8_t e_top = (uint8_t)(e->y >> 8) + e->bbox_y;
        if (e_top > s_bottom) { e++; continue; }

        uint8_t e_bottom = e_top + e->bbox_h;
        if (e_bottom < s_top) { e++; continue; }
        ...
        e->animSpeed = 5;
        e->color = VCOL_YELLOW;
        vspr_color(8 + i, VCOL_YELLOW);

        score += enemyCatalog[e->typeID].points;
        rirq_wait();
        update_score();
...
void init_level_complete() {
    ...
    memset(enemyPool, 0, sizeof(enemyPool));
    memset(enemyBulletPool, 0, sizeof(enemyBulletPool));
    memset(bulletPool, 0, sizeof(bulletPool));

    satellite.active = false;
    for (uint8_t i = 0; i < VSPRITES_MAX; i++) {
        vspr_hide(i);
    }
...
void init_gamestate_title() {
    ...
    vic_setmode(VICM_TEXT_MC, Screen, Charset3);
...
void draw_logo_row() {
    const char* mp = LogoMap + logo_map_y*40;
    char* sp = Screen;
...
void scroll_logo_down() {
    ...
    if(logo_map_y>=0) {
        ...
        if(scrollY == 8) {
            scrollY = 0;
            vic.ctrl1 = VIC_CTRL1_DEN | (scrollY & 7);
...
void draw_credits() {
    if (draw_credits_done == 1) return;
    
    const char* mp = CreditsMap + 40*14;
    char* sp = Screen + 40*14;
    char* cm = Color +40*14;
    char tile;
...
void fill_column_stars() {
    ...
    char * sp = Screen + 39;       
    unsigned sx = starsX >> 11;
    unsigned sy = 0;

    for(char y=0; y<13; y++) {
        char tile = StarsMap[sx + 40*sy];
