 ...
 *
 * Chapter08: Sprite-Sprite HW Collision 
 * 
 * @copyright
 * All rights reserved. 
...
void check_bullet_enemy_coll_hw();
...
void check_bullet_enemy_coll_hw() {
    uint8_t collisions = vic.spr_sprcol;

    if ((collisions & 0x14)==0x14)  {  // (sprite 2 and sprite 4)
        spr_show(2,false);
        bulletPool[0].active = 0;
        spr_show(4,false);
        enemyPool[0].active = 0;
        return;
    }
    if ((collisions & 0x18)==0x18)  {  // (sprite 3 and sprite 4)
        spr_show(3,false);
        bulletPool[1].active = 0;
        spr_show(4,false);
        enemyPool[0].active = 0;
        return;
    }

    if ((collisions & 0x24)==0x24)  {  // (sprite 2 and sprite 5)
        spr_show(2,false);
        bulletPool[0].active = 0;
        spr_show(5,false);
        enemyPool[1].active = 0;
        return;
    }
    if ((collisions & 0x28)==0x28)  {  // (sprite 3 and sprite 5)
        spr_show(3,false);
        bulletPool[1].active = 0;
        spr_show(5,false);
        enemyPool[1].active = 0;
