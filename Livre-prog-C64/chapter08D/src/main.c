 ...
 *
 * Chapter08D: player's ship-background hw collisions 
 * 
 * @copyright
 * All rights reserved. 
...
void check_player_wall_collision();
...
void update_player_ship() {
    if (player_ship.state == STATE_ALIVE) {
        ...
        if (player_ship.currentFrame >= player_ship.maxFrames) {           
            ...
            spr_image(0, sprite_base + player_ship.base_frame + player_ship.currentFrame);
    }
}
...
void check_bullets_enemies_collisions2() {
    for (uint8_t b = 0; b < MAX_BULLETS; b++) {
        if (!bulletPool[b].active) continue;

        uint16_t bx = bulletPool[b].x;
        uint8_t  by = bulletPool[b].y;
...
void handle_enemy_hit(uint8_t eIdx, uint8_t bIdx) {
    ...
    if (enemyPool[eIdx].hp <= 0 && enemyPool[eIdx].state == STATE_ALIVE) {
        ...
        spr_color(4+eIdx,enemyPool[eIdx].color);
    }

    bulletPool[bIdx].active = 0;
    spr_show(2+bIdx,false);
}
...
void spawn_player_explosion() {
    ...
    spr_color(0,player_ship.color);
    spr_show(1,false);
}

void check_player_wall_collision() {

    if (player_ship.state == STATE_EXPLODING)
        return;

    uint8_t bg_collisions = vic.spr_backcol;

    if (bg_collisions & 0x01) {
        spawn_player_explosion();
    }
}
